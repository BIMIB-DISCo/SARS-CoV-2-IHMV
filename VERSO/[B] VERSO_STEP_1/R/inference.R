...
learn.longitudinal.phylogeny <- function(D,
    ...
    for (i in 1:num_rs) {
        ...
        if(verbose) {
            cat(paste0("Performing restart number ",
                       as.character(as.integer(i)),
                       " out of ",
                       as.character(as.integer(num_rs))),
                "\n")
        }
        ...
        if (keep_equivalent) {
            ...
            equivalent_solutions[[1]] <-
                list(B = B_best,
                     C = C_best,
                     relative_likelihoods = lik_best,
                     joint_likelihood = joint_lik_best)
        }
        ...
        for (j in 1:num_iter) {
            ...
            if (verbose && (j %% 100) == 0) {
                cat(paste0("Performed iteration number ",
                           as.character(as.integer(j)),
                           " out of ",
                           as.character(as.integer(num_iter)),
            ...
            ## If likelihood at current step is better than best
            ## likelihood, replace best model with current.
            if (joint_lik_tmp > joint_lik_best) {
                ...
                if (joint_lik_tmp == joint_lik_best) {
                    if (keep_equivalent) {
                        equivalent_solutions[[(length(equivalent_solutions)+1)]] <-
                            list(B = B_tmp,
                                 C = C_tmp,
                                 relative_likelihoods = lik_tmp,
                                 joint_likelihood = joint_lik_tmp)
                    }
                }
                ...
                if (count_lik_best_cons > n_try_bs) {
                    ...
                    ## Warning message
                    if (verbose) {
                        cat(paste0("Not improving likelihood of best solution after ",
                                   as.character(as.integer(n_try_bs)),
                                   " iterations. Skipping to next restart.\n"))
                    }
                    break
                }
                
                ## Take the current state with a probability
                ## proportional to the ratio of the two likelihoods
    ...
    return(list(B = B_global,
                C = C_global,
                lik = lik_global,
                joint_lik = joint_lik_global,
                equivalent_solutions = equivalent_solutions_global))
}
...
move.B <- function(B, seed = NULL ) {
    ...
    ## Perform structural moves with 40% probability
        ...
        while (is_not_valid) {
            ...
            ## A pair of two nodes is valid if the nodes are not
            ## already directly connected
            
            if (!(all(B[ch_1, 1:ch_2] == B[ch_2,1:ch_2]) &&
                  sum(B[ch_1, ]) == (sum(B[ch_2, ]) + 1))) {
...
compute.C <- function(B,
    ...
    for (i in 1:length(D)) {
        ...
        for(k in 2:nrow(B)) {
            ...
            ## Factorization D = C dot B. r_D_tilde represents a
            ## combination of mutations and clones
            
            r_D_tilde <- (curr_clone_C %*% B[-1, -1, drop = FALSE]) * 2
            
            sum_cell_clone <-
                as.matrix(sweep(curr_cells_D, MARGIN = 2, r_D_tilde, "+"))
            lik_matrix[,(k-1)] <-
                (curr_beta^rowSums(sum_cell_clone == 2)) *
                ((1-curr_beta)^rowSums(sum_cell_clone == 0)) *
                ((curr_alpha)^rowSums(sum_cell_clone == 1)) *
                ((1-curr_alpha)^rowSums(sum_cell_clone == 3))
