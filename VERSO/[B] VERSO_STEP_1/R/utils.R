### utils.R
### Utility functions for longitudinal inference and main VERSO
### function.
###
...
## check.undistinguishable
## Removes any indistinguishable event from input data
    ...
    if (length(indistinguishable) > 0) {
        ...
        for (i in invalid_colnames) {
            for (j in valid_colnames) {
                if (all(is.na(full_data[, i]) == is.na(full_data[, j]))) {

                    ## if NAs in i and j are the same

                    if (all(full_data[, i] == full_data[, j], na.rm = TRUE)) {

                        ## if not NAs entries in i and j are the same
                        
                        new_colnames[which(valid_colnames == j)] <-
                            paste0(new_colnames[which(valid_colnames == j)],
                                   "|",
                                   invalid_colnames[which(invalid_colnames == i)])
                        next
                    }
                }
            }
...
### as.adj.matrix
### Build adjacency matrix from B as returned from the function
...
### Build adjacency matrix from B as returned from the function
### learn.longitudinal.phylogeny (unsorted)
...
### as.B
### Builds B from an adjacency matrix where we assume clones and
### mutations to be both ordered
...
### checkQonAB
### Checks that point Q is not on segments A-B
...
### Checks if two segments interserct
...
compute.mutation.distance <- function(inference) {
    ...
    mutation_distance <- as.matrix(dist(t(inference$corrected_genotype),
                                        method = "euclidean"))
