### frontend.R
### Performing inference of the maximum likelihood tree.
###
### See the file LICENSE in the top directory for licensing
### information.
...
VERSO <- function(D,
    ...
    if (typeof(D) == "S4") {
        ...
        for(i in 1:length(curr_experiment_unique)) {
            curr_D[[as.character(curr_experiment_unique[i])]] <-
                curr_data[which(curr_experiment==curr_experiment_unique[i]), , drop = FALSE]
        }
        D <- curr_D
    }
    ...
    if (verbose) {
        cat(paste0("Starting inference for a total of ",
                   length(alpha),
                   " difference values of alpha and beta.",
                   "\n"))
    }
    ...
    if (is.null(parallel) && length(alpha) > 1) {
        ...
        if(verbose && !is.null(parallel)) {
            cat("Executing", num_processes,
                "processes via parallel...",
                "\n")
        }
    }
    ...
    if (is.null(parallel)) {
        ...
        for (i in 1:length(alpha)) {
            ...
            inference[[i]] <-
                learn.longitudinal.phylogeny(D = D, 
                                             lik_w = lik_w, 
                                             alpha = alpha[[i]], 
                                             beta = beta[[i]], 
        ...
        clusterExport(parallel,
                      ...
                      envir = environment())
        clusterExport(parallel,
                      c("learn.longitudinal.phylogeny",
                        "initialize.B",
                        "move.B",
                        "compute.C"),
                      envir = environment())
        clusterSetRNGStream(parallel, iseed = round(runif(1) * 100000))
        inference <- parLapply(parallel,
                               1:length(alpha),
                               function(x) {
                                   if(verbose) {
                                       cat('Performing inference for alpha =',
                                           paste0(alpha[[x]],collapse=" | "),
                                           'and beta =',
    ...
    for (curr_C_index in 1:length(inference_attachments)) {
        inference_attachments_ordered[curr_C_index] <-
            as.integer(colnames(inference_B)[inference_attachments[curr_C_index]])
    }
    ...
    for (i in 1:dim(clones_prevalence)[1]) {
        ...
        for(j in 1:length(C)) {
            clone_number_cell <-
                clone_number_cell + length(which(C[[j]] == i))
            clones_prevalence[i,j] <-
                length(which(C[[j]] == i)) / dim(C[[j]])[1]
        }
    ...
    if (length(equivalent_solutions)>1) {
        ## If we have at least one other solution besides the best one.
        duplicated <- NULL
        for (i in 2:length(equivalent_solutions)) {
            ## Consider all solutions besides the first one
            for (j in 1:i) {
                ## check if we have equivalent solutions among the ones discovered before
                if (i != j) {
        ...
        for (i in 1:length(equivalent_solutions)) {
            rownames(equivalent_solutions[[i]][["B"]]) <-
                c("Root",
                  paste0("Clone_", rownames(equivalent_solutions[[i]][["B"]])[2:nrow(equivalent_solutions[[i]][["B"]])]))
            colnames(equivalent_solutions[[i]][["B"]]) <-
                c("Root",
                  colnames(D[[1]])[as.numeric(colnames(equivalent_solutions[[i]][["B"]])[2:ncol(equivalent_solutions[[i]][["B"]])])])
        }
    }
    if (length(equivalent_solutions)==1) {
        ## If we have only the best solution (no other equivalent solutions)
        equivalent_solutions <- list()
    }

    return(list(B = B,
                C = C,
                corrected_genotypes = corrected_genotypes,
                clones_prevalence = clones_prevalence,
                relative_likelihoods = relative_likelihoods,
                joint_likelihood = joint_likelihood,
                clones_summary = clones_summary,
                equivalent_solutions = equivalent_solutions,
                error_rates = error_rates))
}

### end of file -- frontend.R
